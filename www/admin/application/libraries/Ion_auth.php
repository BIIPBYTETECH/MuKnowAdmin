...
//echo '<pre>'; print_r($email_activation); exit;
...
//echo '<pre>'; print_r($id); exit;
    ...
    public function is_facilitator() {
        ...
        $this->db->where("u.active='1' AND ug.group_id='2' AND u.id=$user_id AND u.user_type='facilitator'");
     ...
     public function is_trainer() {
        ...
        $this->db->from('users u');
        $this->db->join(' users_groups ug', 'ug.user_id = u.id');
        $this->db->where("u.active='1' AND ug.group_id='2' AND u.id=$user_id AND u.user_type='trainer'");
        $result = $this->db->get()->result();
        //echo $this->db->last_query();exit();
        if (!empty($result)):
    ...
    public function is_contributor($id = false) {
        $user_id = $this->user()->row()->id;
        $this->db->select('u.*');
        $this->db->from('users u');
        $this->db->join(' users_groups ug', 'ug.user_id = u.id');
        $this->db->where("u.active='1' AND ug.group_id='2' AND u.id=$user_id AND u.user_type='contributor'");
        $result = $this->db->get()->result();
        if (!empty($result)):
    ...
    public function is_subsciber($id = false) {
        $user_id = $this->user()->row()->id;
        $this->db->select('u.*');
        $this->db->from('users u');
        $this->db->join(' users_groups ug', 'ug.user_id = u.id');
        $this->db->where("u.active='1' AND ug.group_id='2' AND u.id=$user_id AND u.user_type='subscriber'");
        $result = $this->db->get()->result();
        // echo $this->db->last_query();exit();
        if (!empty($result)):
