...
class Dashbord_welcome extends Admin_Controller {
    ...
    public function index() {
        ...
        if ($this->ion_auth->is_admin() ):
            $this->data['approved_articles'] = $this->Articles_model->get_approved_article();
         $this->data['pending_articles'] = $this->Articles_model->get_pending_article();
        else:
            $this->data['approved_articles'] = $this->Articles_model->count_all_results(array('active' => '1', 'author_id' => $user_id, 'deleted' => '0'));
                $this->data['pending_articles'] = $this->Articles_model->count_all_results(array('active' > '0', 'author_id' => $user_id, 'deleted' => '0'));

        endif;
        $this->data['subscriber'] = $this->Register_user_model->get_count_user('subscriber');
        $this->data['unsubscriber'] = $this->Register_user_model->get_count_user('non_subscriber');
       ...
       // echo 'admin_menu_'.$language;exit();
        ...
        $this->lang->load('admin_menu_' . 'english' . '_lang', $language);
