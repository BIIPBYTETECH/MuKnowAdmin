...
class Articles_model extends MY_Model {
    ...
    function get_random_articles($user_type) {
		...
		$sql = "SELECT a.* FROM articles as a, category as c where a.active = '1' AND a.deleted = '0' AND c.id = a.cat_id AND c.deleted = '0' AND c.parent_id != '0' AND a.article_type = '$user_type' AND a.article_type != 'courses' ORDER BY id DESC LIMIT 10;";
        }
        elseif($user_type=='subscriber')
        {
         $sql = "SELECT a.* FROM articles as a, category as c where a.active = '1' AND a.deleted = '0' AND c.id = a.cat_id AND c.deleted = '0' AND c.parent_id != '0' AND a.article_type != 'mini_certification' AND a.article_type != 'courses' ORDER BY id DESC LIMIT 10;";

        }
		else{
			 $sql = "SELECT a.* FROM articles as a, category as c where a.active = '1' AND a.deleted = '0' AND c.id = a.cat_id AND c.deleted = '0' AND c.parent_id != '0' AND a.article_type = 'non_subscriber'  ORDER BY id DESC LIMIT 10;";
    ...
    function get_fav_random_articles($user_type='') {
        ...
        $sql = "SELECT a.* FROM articles as a, category as c, favourite_article as fa 
                where a.active = '1' AND a.deleted = '0' AND c.id = a.cat_id AND c.deleted = '0' AND a.id=fa.article_id AND a.article_type = '$user_type' AND a.article_type != 'courses'
                GROUP BY fa.article_id order by count(fa.article_id) desc limit 10;";
        }
        elseif($user_type=='subscriber')
        {
            $sql = "SELECT a.* FROM articles as a, category as c, favourite_article as fa 
                where a.active = '1' AND a.deleted = '0' AND c.id = a.cat_id AND c.deleted = '0' AND a.id=fa.article_id AND a.article_type != 'mini_certification' AND a.article_type != 'courses'
                GROUP BY fa.article_id order by count(fa.article_id) desc limit 10;";
        }
		else{
			 $sql = "SELECT a.* FROM articles as a, category as c, favourite_article as fa 
                where a.active = '1' AND a.deleted = '0' AND c.id = a.cat_id AND c.deleted = '0' AND a.id=fa.article_id AND a.article_type = '0'
                GROUP BY fa.article_id order by count(fa.article_id) desc limit 10;";
...
//            }
  ...
  function get_article_by_author_row($id) {
    ...
    function get_article_by_author_row_courses($id) {
       // $this->db->where('group_company.company_id',$id);
            $this->db->where('deleted','0');
            $this->db->where('active','1');
            $this->db->where('article_type','courses');
            $this->db->where('author_id',$id);
            return $this->db->get('articles')->row();
	...
	function update_telenor_responses($data)
	{
		 $this->db->insert('telenor_response',$data);
       return $this->db->insert_id();
	}
	function get_telenor_data($mobile)
	{
		
		$this->db->where('mobile', $mobile);
        return $this->db->get('telenor_otp_data')->row();
		
	}
	
	function update_telenor_data($id ,$mobile_otp)
	{
		//echo $id;  echo $mobile_otp; exit;
		$data['mobile_otp'] = $mobile_otp;
		$this->db->where('id',$id);
       $this->db->update('telenor_otp_data',$data);
       return true;
	}
		function get_telenor_subscribed_data($mobile)
	{
		//$this->db->select('*');
		$this->db->where('msisdn', $mobile);
		$this->db->where('charging_type', "ACT");
        return $this->db->get('telenor_response')->row();
		
	}
	function check_telenor_subscribed_or_not($mobile)
	{	
	...
	function get_telenor_old_data($id)
	{

		$this->db->where('id', $id);
		$this->db->where('lifecycle', "A");
        return $this->db->get('telenor_response')->row();
		
	}
	function get_telenor_recents_data($id)
	{
	...
	function get_all_non_and_sub_articles($author_id , $language = null)
    {
		if($language)
		{
			$this->db->where('language_key',$language);
