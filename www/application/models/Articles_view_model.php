...
class Articles_view_model extends MY_Model {
    ...
    function individual_user_article_view() 
    {
        ...
        $sql = "SELECT u.* ,"
                 . "count(DISTINCT av.article_id) y ,av.user_id as x "
                 . "FROM articles_view_count as av JOIN mk_users as u ON u.user_id=av.user_id "
...
//        $this->db->distinct();
        $this->db->select(" a.*,c.*,u.*");


        $this->db->from('articles_view_count av');

       

       
        $this->db->join('articles a ', 'av.article_id = a.id'); 
        $this->db->join(' category c', 'c.id = a.cat_id');
        $this->db->join(' mk_users u', 'u.user_id = av.user_id');
        $this->db->where("av.user_id=$user_id");
