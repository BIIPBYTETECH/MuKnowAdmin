...
class Auth extends CI_Controller {
    ...
    function index() {
        ...
        if (!$this->ion_auth->logged_in()) {
            ...
            redirect('MuKnow_dev_authlogin', 'refresh');
...
// create a new user
    function create_user() { 
      //  echo '<pre>'; print_r($_POST); exit;
        ...
        // validate form input
        $this->form_validation->set_rules('username', 'Full name', 'required|alpha_numeric_spaces');
...
//           $this->form_validation->set_message('username', 'The Full name field may only contain alpha-numeric characters.');
...
//       else
        ...
        if ($identity_column !== 'phone') {
            ...
            if (empty($phone)):
                ...
                if (!empty($exit_phone)):
                    ...
                    $this->form_validation->set_rules('phone', $this->lang->line('create_user_validation_phone_label'), 'required|trim|numeric|min_length[8]|max_length[10]|is_unique[' . $tables['users'] . '.' . $identity_column . ']');
                    $this->form_validation->set_message('is_unique', 'This %s is already registered.');
                else:
                    $this->form_validation->set_rules('phone', $this->lang->line('create_user_validation_phone_label'), 'required|trim|numeric|min_length[8]|max_length[10]');
        ...
        if (!empty($password_confirm)):
            $this->form_validation->set_rules('password', $this->lang->line('create_user_validation_password_label'), 'required|alpha_numeric|min_length[' . $this->config->item('min_password_length', 'ion_auth') . ']|max_length[' . $this->config->item('max_password_length', 'ion_auth') . ']|matches[password_confirm]|callback_password_check');
            $this->form_validation->set_message('password_check', 'Password should be combination of alphanumeric .');
        else:
            $this->form_validation->set_rules('password', $this->lang->line('create_user_validation_password_label'), 'required|alpha_numeric|min_length[' . $this->config->item('min_password_length', 'ion_auth') . ']|max_length[' . $this->config->item('max_password_length', 'ion_auth') . ']');
        endif;
      ...
      //  echo '<pre>'; print_r($additional_data); exit; 
       ...
       // if ($this->form_validation->run() == true && !empty($ids)) { 
          if ($this->form_validation->run() == true) { 
            ...
            redirect(site_url() . 'admin/Dashboard_user_trainers/index/trainer');
        } else {
    ...
    function edit_user($id) {
        ...
        // validate form input
        $this->form_validation->set_rules('first_name', $this->lang->line('edit_user_validation_fname_label'), 'required|alpha_numeric_spaces');
